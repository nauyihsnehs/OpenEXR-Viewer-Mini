...
- Pers-view 左拖改变方向；画布屏幕边界内滚轮调水平 FOV，边界外滚轮缩放画面。默认 +Z、+Y 上方、90°，限制 10°～150°。
- Sphere 是从外部看的正交贴图球，无额外光照或反射；左拖旋转，滚轮缩放，球外棋盘格。
- 两种相机视图共用朝向。投影菜单 Reset 与 28×28 图标回正按钮共用逻辑，恢复 +Z、+Y 向上和 FOV 90°，保持投影类型、缩放及颜色参数。
- RGB、单通道和极简底栏只在这两种视图显示朝向组件：水平角、俯仰角保留一位小数，Pers-view 另显示 FOV。核对数值始终对应已显示画面，右键颜色重置不改变朝向。
- 普通信息栏、画布空白及极简底栏非按钮区域滚轮缩放；输入框、按钮和其他面板不被拦截。检查 Ctrl＋滚轮调颜色，以及 + / − / 0 / 1 缩放快捷键。
- 中键保留普通平移/极简窗口移动。检查明暗主题、高 DPI、极简窗口进出后状态。
- 交互期间合并输入，每模型仅一个在途任务、提交上限 30 次/秒；允许同次交互中已完成的帧先显示，不能因持续取消而一直没有画面。
- 临时采样最长边不超过 512 像素，小画面不放大；仍采样当前源层。检查临时与完整帧的画布、窗口大小、Fit、手动缩放和中心位置一致，读数、球外透明度及异常标记随画面同步。
- 松开拖动立即请求完整帧，FOV 滚轮停止 150 ms 后恢复完整帧；切投影、刷新、换源或结束交互后，旧任务不得覆盖最终画面。
- 色调映射、曝光、Colormap 调整复用同朝向、投影及尺寸的已完成线性投影缓冲；源统计、Auto 和所选层保持不变。
- 连续拖动、快速滚轮、松开后马上回正和切换投影，人工核对出帧、最终清晰度与输入延迟；当前未测量帧率或跟手程度。
...
- StageEnvLatLong 在原生布局、Fit 模式下反复切换第 `(7,7)` 层（8×4）、更小层及完整分辨率。即使倍率超过 6400%，加载结束后层级滑条也应恢复可用、`…` 消失；菜单切层后仍可继续拖动。RGB、独立通道及极简窗口分别核对，并检查加载失败后的恢复。
...
- 交互中及完整帧恢复前禁用复制、预览与转换保存，原始导出仍可用；恢复后全尺寸复制不能取得 512 像素临时帧。
...
- 保存对话框捕获已完成画面；若打开时仍在渲染，预览/转换保存等待完整帧后捕获一次，随后固定源、层级和显示参数。

## 静态验证范围

- 已补未执行的回归用例：六方向及面角、负起点、视场角、Sphere 透明度、退化层、异常分类和取消；
  源缓冲不变、线性 EXR/色域/envmap、PNG/JPEG、标量名称与覆盖 Alpha、菜单/刷新/极简恢复。
- 新增但未执行：区域滚轮、FOV 边界与快捷键、底栏缩放及按钮排除、朝向和回正、交互帧不饿死、最终帧尺寸、稳定画布、同步透明度/读数、输出就绪限制和颜色缓存复用。
- 层级滑条的鼠标事件顺序、高倍率 Fit 及加载/取消后的启用恢复已补回归代码，未运行；上述 StageEnvLatLong 操作仍待人工确认。
